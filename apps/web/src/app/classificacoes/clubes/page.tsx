...
export const metadata = { title: "Classificação por clube" };
...
export default async function ClubRankingsPage() {
  ...
  const snapshots = await Promise.all(classes.map((raceClass) => prisma.rankingSnapshot.findFirst({
    where: { scope: "CLUB", classId: raceClass.id, status: { in: ["PROVISIONAL", "DEFINITIVE"] } },
  ...
  return <><SiteHeader /><main className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
    ...
    <p className="mt-6 text-sm font-black uppercase tracking-[0.15em] text-brand-red">Comunidades náuticas</p>
    <h1 className="mt-1 text-4xl font-black text-brand-navy">Classificação por clube</h1>
    <p className="mt-3 max-w-3xl text-slate-600">Média dos dez melhores participantes de cada clube, ou de todos quando existirem menos de dez.</p>
